...
import type { ChatMessageDto, ExtensionToWebview, SessionDto } from '../../src/shared/types/webview.types';
...
describe('host → webview message contract', () => {
  ...
  it('keeps every payload structured-clone safe', () => {
    ...
    const message: ExtensionToWebview = {
      type: 'agentStream',
      runId: 'r1',
      steps: [{ kind: 'tool', toolCallId: 't1', name: 'glob', status: 'ok', preview: 'x' }],
    };
